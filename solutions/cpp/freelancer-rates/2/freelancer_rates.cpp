...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    double before_discount = daily_rate(hourly_rate) * 22;

    return apply_discount(before_discount, discount);
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    int budget_before_discount = budget/(1 - discount/100);
    int day_rate = daily_rate(hourly_rate);
    int num_days = budget_before_discount/day_rate;
